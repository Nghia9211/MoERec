...
from core.candidate_retriever  import CandidateRetriever
from gating.gating_network     import GatingNetwork
from core.moe_fusion           import MoEFusion
...
class MoERecAgent:
    ...
    def act(self, data: dict, reason: str = None, item: str = None, epoch: int = None, rejected_items: List[str] = None) -> Tuple[str, List[str], dict]:
        ...
        try:
            ...
            # Top-K per expert within the top-M pool
            def _expert_topk(expert_key: str, items: list, k: int = 5) -> list:
                """Return top-k items from pool ranked by a single expert's score."""
            ...
            # Ablation mode: skip reranker and return MoE output directly
