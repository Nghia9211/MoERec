...
def split_rec_reponse(response):
    """Parse rec-agent response in the form: Item: <single item>."""
    if response is None:
...
def split_user_response(response):
    """
    Parse user-agent response in the form:
        Reason: <multi-line text>
        Decision: yes/no

    Robust handling:
      - Multi-line Reason (POSITIVE MATCHES, NEGATIVE NOISE, ...)
      - Decision anywhere after Reason
      - Extra whitespace/newlines around keywords
      - Decision with trailing punctuation (e.g. "Yes.", "No (reason)")
    """
    if response is None:
    ...
    # Find Decision (search from end to avoid false positives in Reason)
    ...
    if not decision_match:
        # Fallback: yes/no alone on the last line
    ...
    # Extract Reason: from after "Reason:" to before "Decision:"
    ...
    if reason_match:
        ...
        reason = text   # fallback: entire response as reason
